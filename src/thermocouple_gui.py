...
def load_firebase_config(root):
    """
    Ensure firebase_config.json exists; if not, prompt the user to select
    a service account JSON and enter the database URL, then save them.
    ...
    dialog.title("Firebase Setup")
    ttk.Label(dialog, text="Select Service Account JSON:").grid(row=0, column=0, sticky="w", padx=5, pady=5)
    sa_entry = ttk.Entry(dialog, width=40)
    sa_entry.grid(row=0, column=1, padx=5, pady=5)
    def browse_sa():
        path = filedialog.askopenfilename(filetypes=[("JSON", "*.json")])
        if path:
            sa_entry.delete(0, tk.END)
            sa_entry.insert(0, path)
    ttk.Button(dialog, text="Browse...", command=browse_sa).grid(row=0, column=2, padx=5, pady=5)

    ttk.Label(dialog, text="Database URL:").grid(row=1, column=0, sticky="w", padx=5, pady=5)
    url_entry = ttk.Entry(dialog, width=40)
    url_entry.grid(row=1, column=1, columnspan=2, padx=5, pady=5)

    config = {}
    def on_ok():
        config['serviceAccount'] = sa_entry.get().strip()
        config['databaseURL'] = url_entry.get().strip()
        with open(firebase_config_file, "w") as f:
            json.dump(config, f, indent=2)
        dialog.destroy()

    ttk.Button(dialog, text="OK", command=on_ok).grid(row=2, column=1, pady=10)
        ...
        try:
            ...
            try:
                firebase_db_ref.push({
                    ...
                    'cpu_temp': cj_temp,
                    'raw_temp': raw_temp,
                    'corrected_temp': corrected
                })
            except Exception as fb_e:
                print(f"Firebase push error: {fb_e}")
...
def launch_gui():
    global running, paused, firebase_initialized
    root = tk.Tk()
    ...
    # Load or prompt for Firebase configuration
    firebase_config = load_firebase_config(root)
